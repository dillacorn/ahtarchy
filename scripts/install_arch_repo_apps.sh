...
if [[ "$choice" == "y" || "$choice" == "Y" ]]; then
    ...
    for pkg in hyprland hyprpaper hyprlock hypridle waybar rofi-wayland swww grim slurp wl-clipboard qt5-wayland qt6-wayland xdg-desktop-portal-hyprland xdg-desktop-portal-gtk mako; do
        install_package "$pkg"
    done
    ...
    for pkg in steam polkit-kde-agent lxappearance networkmanager network-manager-applet tailscale bluez bluez-utils blueman pavucontrol pcmanfm gvfs gvfs-smb gvfs-mtp gvfs-afc qbittorrent filelight timeshift flameshot imagemagick pipewire pipewire-pulse pipewire-alsa; do
        install_package "$pkg"
    done
