...
if [[ "$choice" == "y" || "$choice" == "Y" ]]; then
    ...
    for pkg in hyprland hyprpaper hyprlock hypridle waybar rofi-wayland swww grim satty slurp wl-clipboard qt5-wayland qt6-wayland xdg-desktop-portal-hyprland xdg-desktop-portal-gtk mako nwg-look; do
        install_package "$pkg"
    done
    ...
    for pkg in micro alacritty fastfetch btop htop curl wget git dos2unix brightnessctl ipcalc cmatrix sl asciiquarium figlet cava; do
        install_package "$pkg"
    done

    # Install Utilities
    echo -e "${CYAN}Installing general utilities...${NC}"
    for pkg in steam polkit-kde-agent lxappearance networkmanager network-manager-applet tailscale bluez bluez-utils blueman pavucontrol pcmanfm gvfs gvfs-smb gvfs-mtp gvfs-afc qbittorrent filelight timeshift imagemagick pipewire pipewire-pulse pipewire-alsa ufw; do
        install_package "$pkg"
    done
